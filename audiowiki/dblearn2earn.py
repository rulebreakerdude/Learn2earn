...
#hiv specific definitions start here
    ...
    def addHivResponse(self,user,question,response,datetime):
        self.c.execute("INSERT INTO hivResponses (user, question, response, datetime) VALUES (%s, %s, %s, %s);" ,(str(user), str(question), str(response), str(datetime)))
        self.db.commit()
        
    def addHivReferral(self,user,referral,datetime):
        self.c.execute("INSERT INTO hivReferral (user, referral, datetime) VALUES (%s, %s, %s);" ,(str(user), str(referral), str(datetime)))
        self.db.commit()
        
    def addHivPair(self,user,referred_by,datetime):
        self.c.execute("INSERT INTO hivPair (user, referred_by, datetime) VALUES (%s, %s, %s);" ,(str(user), str(referred_by), str(datetime)))
        self.db.commit()

    def addHivRechargeStatus(self,user,status,datetime):
        self.c.execute("INSERT INTO hivRecharge (user, status, datetime) VALUES (%s, %s, %s);" ,(str(user), str(status), str(datetime)))
        self.db.commit()    
        
    def addHivRechargeSuccessStatus(self,user,operator,status,datetime):
        self.c.execute("INSERT INTO hivRechargeSuccess (user, operator, status, datetime) VALUES (%s, %s, %s, %s);" ,(str(user), str(operator), str(status), str(datetime)))
        self.db.commit() 
        
    def addHivRechargeRider(self,user,operator,status,datetime):
        self.c.execute("INSERT INTO hivRechargeRider (user, operator, status, datetime) VALUES (%s, %s, %s, %s);" ,(str(user), str(operator), str(status), str(datetime)))
        self.db.commit() 

    def addHivPoints(self,user,points,datetime):
        self.c.execute("INSERT INTO hivPoints (user, points, datetime) VALUES (%s, %s, %s);" ,(str(user), str(points), str(datetime)))
        self.db.commit()
        
    def isHivRechargedSuccess(self, user):
        count = self.c.execute(
            "SELECT DISTINCT status FROM hivRechargeSuccess WHERE user = %s;"
                                            ,(str(user),))
        return count>0
        
    def isHivPair(self, user, referred_by):
        count = self.c.execute(
            "SELECT DISTINCT datetime FROM hivPair WHERE user = %s AND referred_by = %s;"
                                            ,(str(user),str(referred_by)))
        return count>0
        
    def isHivAlreadyReferred(self, user):
        count = self.c.execute(
            "SELECT DISTINCT datetime FROM hivPair WHERE user = %s;"
                                            ,(str(user),))
        return count>0

    def isHivReferral(self, referral):
        count = self.c.execute(
            "SELECT DISTINCT user FROM hivReferral WHERE referral = %s;"
                                            ,(str(referral),))
        return count>0  
        
    def isHivRecharged(self, user):
        count = self.c.execute(
            "SELECT DISTINCT status FROM hivRecharge WHERE user = %s;"
                                            ,(str(user),))
        return count>0

    def doesUserHaveReferral(self, user):
        count = self.c.execute(
            "SELECT DISTINCT referral FROM hivReferral WHERE user = %s;"
                                            ,(str(user),))
        return count>0  

    def userAgainstReferral(self, referral):
        r_user = self.c.execute(
            "SELECT DISTINCT user FROM hivReferral WHERE referral= %s;"
                                            ,(str(referral),))
        r_user = self.c.fetchall()
        r_user =[i[0] for i in r_user]
        return r_user[0]
        
    def referralAgainstUser(self, user):
        r_referral = self.c.execute("SELECT DISTINCT referral FROM hivReferral WHERE user= %s;",(str(user),))
        r_referral = self.c.fetchall()
        r_referral =[i[0] for i in r_referral]
        return r_referral[0]
        
    def operatorAgainstUser(self, user):
        r_operator = self.c.execute(
            "SELECT DISTINCT operator FROM hivRechargeSuccess WHERE user= %s;"
                                            ,(str(user),))
        r_operator = self.c.fetchall()
        r_operator =[i[0] for i in r_operator]
        return r_operator[0]
        
    def getSessionID(self):
        r_SessionID = self.c.execute(
            "SELECT DISTINCT SessionID FROM hivSession ORDER BY SessionID DESC;",)
        r_SessionID = self.c.fetchall()
        r_SessionID =[i[0] for i in r_SessionID]
        return r_SessionID[0]
        
    def getFailedRecharges(self,datetime1,datetime2):
        r_user = self.c.execute(
            "SELECT DISTINCT user FROM hivRecharge WHERE datetime >= %s AND datetime <= %s AND status LIKE \"FAILED%%\";",(str(datetime1),str(datetime2),))
        r_user = self.c.fetchall()
        r_user =[i[0] for i in r_user]
        return r_user
        
    def getSuccessfulRecharges(self,datetime1,datetime2):
        r_user = self.c.execute(
            "SELECT DISTINCT user FROM hivRechargeSuccess WHERE datetime >= %s AND datetime <= %s;",(str(datetime1),str(datetime2),))
        r_user = self.c.fetchall()
        r_user =[i[0] for i in r_user]
        return r_user
        
    def addCronLog(self,datetime):
        self.c.execute("INSERT INTO hivCronLog (datetime) VALUES (%s);" ,(str(datetime),))
        self.db.commit()
        
#hiv specific definitions end here
