...
def child(l1):
    ...
    for i in l1:
        file1 = file1+i
    time.sleep(2);
    call_value = chk_call(file1);
    print "Return sucessfull";
    if(int(call_value) == 0):
        print "The call value in child is : ",call_value
        print "establishing connection ...";
        db = MySQLdb.connect("127.0.0.1","root","Wmtp00lr!","audiwikiswara");
        print "connection established"; 
        cursor = db.cursor();
        fname = file1.split("/")[0];
        print "got the filename and the cursor";
        qry = "update lb_postings set status=7 where audio_file = '"+fname+"';";
        print "qry is";
        print qry;
        cursor.execute(qry);
        print "qry executed";
...
def playFile (fname, keyDict = newKeyDict()):
    ...
    if (not isinstance(result,str)) or result == '-1':
        ...
        if not (isinstance(keyDict[c],tuple)):
            keyDict[c]()
        else:
...
def recordFile (fname, stopDigits, timeout, silenceTimeout):
    ...
    #f.write(fname);
    #l1 = [str(fname)]
    #p = Process(target=child, args=(l1))
    #p.start()
